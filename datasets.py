...
class DriveDataset(Dataset):
    def __init__(self, root: str, train: bool, transforms=None):
        ...
        if train:
            self.manual = [os.path.join(data_root, "labels", i.replace('training.tif', 'manual1.gif')) for i in img_names]
        else:
            self.manual = [os.path.join(data_root, "1st_manual", i.replace('test.tif', 'manual1.gif')) for i in img_names]
        # check files
...
class MMS_P3_385_385_3_Dataset(Dataset):
    def __init__(self, root: str, train: bool, transforms=None):
        super(MMS_P3_385_385_3_Dataset, self).__init__()
        data_root = os.path.join(root, "aug" if train else "test")
        assert os.path.exists(data_root), f"path '{data_root}' does not exists."
        self.transforms = transforms
        img_names = [i for i in os.listdir(os.path.join(data_root, "images"))]
        self.img_list = [os.path.join(data_root, "images", i) for i in img_names]
        if train:
            self.manual = [os.path.join(data_root, "labels", i.replace('_P3.bmp', '_ref.bmp')) for i in img_names]
        else:
            self.manual = [os.path.join(data_root, "manual", i.replace('_P3.bmp', '_ref.bmp')) for i in img_names]
        # check files
        for i in self.manual:
            if os.path.exists(i) is False:
                print(f"file {i} does not exists.")
